...
submit($command);
$command=join ("","samtools sort -\@ ",$THREADS," -n ",$human_mapping,"/human.bam -o ",$human_mapping,"/human.sort.bam -T ",$human_mapping,"/human.sorting");
submit($command);
...
submit($command);
$command=join ("","samtools sort -@ ",$THREADS," -n ",$human_mapping_again,"/2_human.bam -T ",$human_mapping_again,"/human.sort -o ",$human_mapping_again,"/human.sort.bam");
submit($command);
...
print FH "pid1=\$!\n";
print FH "samtools sort -@ $THREADS $scoring/virus.fromUnmapped.bam -T $scoring/virus.fromUnmapped.sort -o $scoring/virus.fromUnmapped.sort.bam \&\n";
print FH "pid2=\$!\n";
...
submit($command);
$command=join ("","samtools sort -@ ",$THREADS," -n ",$viral_mapping,"/2_virus.bam -T ",$viral_mapping,"/virus.sort -o ",$viral_mapping,"/virus.sort.bam");
submit($command);
...
submit($command);
$command=join ("","samtools sort -@ ",$THREADS," ",$OUTPUT,"/VIRUS.HUMAN.bam -T ",$OUTPUT,"/VIRUS.HUMAN.sort -o ",$OUTPUT,"/VIRUS.HUMAN.sort.bam");
submit($command);
...
submit($command); 
$command=join ("","samtools sort -@ ",$THREADS," ",$OUTPUT,"/merged.bam -T ",$OUTPUT,"/",$SAMPLE,".forcalling -o ",$OUTPUT,"/",$SAMPLE,".forcalling.bam");
submit($command); 
