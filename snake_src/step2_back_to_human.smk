rule back_to_human_to_fastq:
    input:   join(human_mapping_again, 'to_human.sort.bam')
    output:  read1 = join(human_mapping_again, 'read1.fq'),
             read2 = join(human_mapping_again, 'read2.fq')
    shell:   to_fastq_shell


rule back_to_human_sortname:
    input:   join(human_mapping_again, 'to_human.bam')
    output:  join(human_mapping_again, 'to_human.sort.bam')
    threads: config['threads']
    shell:   sortname_shell


rule back_to_human_merge:
    input:   join(human_mapping_again, 'mate_unmapped.bam'),
             join(human_mapping_again, 'mate_mapped.bam')
    output:  join(human_mapping_again, 'to_human.bam')
    shell:   merge_shell


rule back_to_human_extract_mate_unmapped:
